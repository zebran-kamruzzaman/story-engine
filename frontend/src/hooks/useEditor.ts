...
import type { CharacterInteraction } from '../types'
...
export function useEditor() {
  ...
  const loadScene = useCallback(
    async (id: string, cursorPos: number, scrollTop: number) => {
      ...
      // Load last-known mirror state for this scene.
      try {
        const raw = await GetInsights(id)
        let interactions: CharacterInteraction[] = []
        try {
          interactions = JSON.parse(raw.interactionsJSON ?? '[]')
        } catch {
          interactions = []
        }
        setMirror({
          ...
          interactions,
          sceneTone: raw.sceneTone ?? '',
          source: (raw.source ?? 'rule') as 'rule' | 'llm',
        })
      } catch (err) {
